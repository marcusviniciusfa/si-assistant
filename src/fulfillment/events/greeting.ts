...
function greeting (conv: ConversationV3) {
  ...
  const time = subtract(5, 'minutes')

  console.log(conv)
  console.log('conv._internal.orig.user:', conv._internal.orig.user)

  if (lastInteraction <= time) {
    const ssml = '<speak>Olá! Sou a <emphasis level="strong">Sisin</emphasis>, Assistente de voz do curso Sistemas de Informação. <p>Posso ajudar você com assuntos do curso.</p></speak>'
    conv.add(ssml)
